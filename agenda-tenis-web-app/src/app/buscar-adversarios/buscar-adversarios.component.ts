...
import { AdversariosService } from './adversarios.service';
...
export class BuscarAdversariosComponent implements OnInit {
  ...
  buscarAdversarios(): void {
    ...
    this.adversariosService.buscarAdversarios(this.pagina, this.itensPorPagina, idCidadeStr, this.categoria).subscribe(response => {
  ...
  selecionarCidadeModal(cidade: Cidade): void { // Novo método para selecionar cidade no modal
